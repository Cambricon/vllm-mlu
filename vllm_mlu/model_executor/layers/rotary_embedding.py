...
from vllm.model_executor.layers.rotary_embedding import (
    ...
    NTKScalingRotaryEmbedding, DualChunkRotaryEmbedding,
...
from vllm_mlu.v1.attention.backends.utils import (MLUCommonAttentionMetadata,
                                                  get_common_metadata)
from vllm_mlu.v1.attention.backends.mla.flashmla import FlashMLAMetadata
...
class MLURotaryEmbedding(RotaryEmbedding, CustomOp):
    ...
    def __init__(
        ...
        self.use_direct_call = False if envs.VLLM_USE_V1 else True
    ...
    def set_mlu_var(
        ...
        if decode_metadata:
            if prefill_metadata:
                ...
                cu_seq_lens = torch.arange(0, input_ids.shape[0] + 1, 1, dtype=torch.int32,
        ...
        cls.chunked_prefill_enabled = attn_metadata.chunked_prefill_enabled
        if prefill_metadata:
    ...
    def forward_impl(
        ...
        if envs.VLLM_USE_V1:
            ...
            cu_seq_lens_ = MLURotaryEmbedding.cu_seq_lens
    ...
    def forward_oot(
        ...
        if self.use_direct_call:
            return self.forward_impl(positions, x, offsets)
        else:
            torch.ops.vllm.rope_forward(positions, x, self.layer_name, offsets)


def rope_forward(positions: torch.Tensor, x: torch.Tensor, layer_name: str,
...
def rope_forward_fake(positions: torch.Tensor, x: torch.Tensor, layer_name: str,
                      offsets: Optional[torch.Tensor] = None) -> None:
    return
...
class MLULinearScalingRotaryEmbedding(MLURotaryEmbedding, LinearScalingRotaryEmbedding):
    ...
    def _compute_inv_freq(self, base: Union[int, float]) -> torch.Tensor:
        ...
        if self.is_neox_style:
            inv_freq = 1.0 / (base ** ((torch.arange(
                0, self.rotary_dim, 1, dtype=torch.float32, device=current_platform.device_type) %
                half_dim) * 2 / self.rotary_dim)
            )
        else:
            inv_freq = 1.0 / (
                    base
                    ** (torch.arange(0, self.rotary_dim, 1, device=current_platform.device_type,
                        dtype=torch.float32) // 2 * 2 / self.rotary_dim
                    )
            )
        return inv_freq
    ...
    def _compute_cos_sin_cache(self) -> torch.Tensor:
        ...
        for scaling_factor in self.scaling_factors:
            ...
            t = torch.arange(max_len, dtype=torch.float, device=current_platform.device_type)
            t = t / scaling_factor
...
class MLUDeepseekScalingRotaryEmbedding(MLURotaryEmbedding, DeepseekScalingRotaryEmbedding):
    ...
    def _compute_inv_freq(self, scaling_factor: float) -> torch.Tensor:
        ...
        inv_freq_mask = ((1 - _yarn_linear_ramp_mask(
            ...
            dtype=torch.float)) * self.extrapolation_factor).to(current_platform.device_type)
...
class MLUMRotaryEmbedding(MLURotaryEmbedding, MRotaryEmbedding):
    ...
    def __init__(
        ...
        self.register_buffer("tmp_cu_seq_lens", torch.zeros(2, dtype=torch.int32), persistent=False)
        self.mrope_cos_cache = None
        self.mrope_sin_cache = None
        self.mrope_cu_seq_lens = None
    ...
    def precompute_sin_cos_cache(
        ...
        if not MLURotaryEmbedding.is_prompt and not MLURotaryEmbedding.is_chunked:
            return
    ...
    def forward_oot(
        ...
        assert positions.ndim == 1 or positions.ndim == 2
        if not MLURotaryEmbedding.is_prompt and not MLURotaryEmbedding.is_chunked:
            return MLURotaryEmbedding.forward_oot(
                self, positions[0] if positions.ndim == 2 else positions, x)
