...
class SparseMoeMlp(nn.Module):
    ...
    def __init__(
        ...
        self.precompute_dim_bytes_list: Optional[List[int]] = None
        ...
        if VLLM_AVG_MOE_EN and not SparseMoeMlp.is_expert_avg:
            ...
            if VLLM_RANDOM_MOE_EN:
                ...
                if self.use_all2all:
                    ...
                    fix_expert_id = table.reshape(-1, self.dp_size, tp_size)
                    for i in range(self.dp_size):
                        table_dp = fix_expert_id[:, i : i+1, :].contiguous().flatten()\
                                   [:n_tokens//self.dp_size*top_k].view(n_tokens // self.dp_size, top_k)
                        tables.append(table_dp)
                    SparseMoeMlp.expert_id_dp = tables
        ...
        assert not (self.is_fp8_block_wise and self.moe_ep_size > 1), (
            "cambricon_custom_func ep parallel not support fp8 block_wise")
    ...
    def precompute_weight_expert_id(
        ...
        out = [ hidden_states | weight | expert_id ]
              [     bf16      |  fp32  |   int32   ]
        '''
        assert self.is_use_fused_moe and self.expert_group != 1
        assert not self.is_fp8_block_wise, \
            "cambricon_custom_func expert parallel not support fp8 block_wise"
    ...
    def forward_experts(self, hidden_states, expert_logits, residual: Optional[torch.Tensor] = None,
        ...
        use_forward_group_experts = (self.is_use_fused_moe
                                     and (self.is_kimi_k2 or self.expert_group != 1))
    ...
    def forward_group_experts(self, hidden_states, gating_output, residual_):
        ...
        if not is_precompute_weight_expert_id:
            ...
            if self.scoring_func == "softmax":
                ...
                if self.expert_group == 1:
                    # tmo moe_sigmoid_topk does not support expert_group=1 with score_bias for now
                    reduce_weight, expert_id = grouped_topk(hidden_states, gating_output, topk, renormalized,
                                                            self.expert_group, self.topk_group, self.scoring_func,
                                                            self.gate.e_score_correction_bias)
                    reduce_weight *= self.routed_scaling_factor
                else:
