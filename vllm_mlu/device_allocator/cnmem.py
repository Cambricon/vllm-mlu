...
try:
    from vllm_mlu.vllm_mlu_C import (init_module, python_create_and_map, python_unmap_and_release, python_cn_memcpy)
...
def get_pluggable_allocator(
    ...
    new_alloc = torch.mlu.memory.MLUPluggableAllocator(lib_name, 'my_malloc', 'my_free')
    return new_alloc
...
class CnMemAllocator:
    ...
    will be loaded back to MLU memory, and the rest of the tensors will
    have empty memory.
    ...
    def wake_up(self, tags: Optional[list[str]] = None) -> None:
        ...
        All data that is previously offloaded will be loaded back to MLU
        memory, and the rest of the data will have empty memory.

        :param tags: The tags of the memory allocation that will be loaded
            back to MLU memory. If None, all memory allocation will be loaded
            back to MLU memory.
        """
        for ptr, data in self.pointer_to_data.items():
